...
router.get("/playergames", readPlayerGame);
...
function readPlayer(req, res, next) {
    db.oneOrNone(`SELECT * FROM Player, Game WHERE Player.id = Game.id`)
        .then(data => {
...
function readPlayerGame(req, res, next) {
    db.many(`SELECT * FROM Player, PlayerGame WHERE Player.id = PlayerGame.playerid AND PlayerGame.score > 500`)
        .then(data => {
            res.send(data);
        })
        .catch(err => {
